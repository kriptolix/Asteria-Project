...
> **Known inconsistency:** `lang` is missing from `taxonomy_index.html`,
> `taxonomy_term.html` and `404.html`'s context — only `page.html`/`post.html`/`blog.html`
> get it today. If a theme needs the current language on those pages (e.g.
> for a `<html lang="...">` attribute), read it off `site.default_language`
> as a fallback, or flag it so it can be added consistently everywhere.